...
Because the account is currently locked by Snowflake, any programmatic or web connection attempt using standard password authentication is blocked until the lock period expires.
...
2. **Manual Admin Unlock (Alternative)**:
   - If you have administrative access in Snowflake, run the following SQL command in Snowsight:
     ```sql
     ALTER USER <SNOWFLAKE_USER> UNSET IS_LOCKED;
     ```
